...
TEST_CASE("Inorder Traversal", "[data structures][binary tree]") {
	Solution1 solution1;
	Solution2 solution2;
  ...
  SECTION("recursive preorder traversal") {
    REQUIRE_THAT(solution1.inorderTraversal(root), Catch::Matchers::Equals(expected));
  }
  SECTION("iterative Preorder Traversal") {
    REQUIRE_THAT(solution2.inorderTraversal(root), Catch::Matchers::Equals(expected));
  }
}
