...
int find_index_of_rotation(std::vector<int>& nums) {
    int left = 0;
...
int search(std::vector<int>& nums, int target) {
    ...
    int indexOfRotation = find_index_of_rotation(nums);

    if (indexOfRotation == 0) {
        return binary_search(nums, 0, nums.size() - 1, target);
    } else {
        if (target >= nums[0]) {
            return binary_search(nums, 0, indexOfRotation - 1, target);
        } else {
            return binary_search(nums, indexOfRotation, nums.size() - 1, target);
        }
    }
